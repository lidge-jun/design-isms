...
`references.html`은 다음 ISM 확장을 위한 research queue입니다. 타이포그래피, 역사적 디자인 운동, 공식 디자인 시스템 레퍼런스를 ISM 후보와 reference 항목으로 나누고, Grok 리서치 프롬프트와 ima2 이미지 프롬프트를 함께 고정합니다.
...
- References page with 8 ISM candidates and 8 official reference systems
- 32 generated PNG research/reference images and matching WebP previews
- Grok research prompts and ima2 prompt manifests for the next expansion batch
...
- The shared top navigation is duplicated in static HTML and must stay consistent across `index.html`, `effects.html`, and `references.html`.
...
- Reference/research images keep originals at `assets/images/{candidate-id}/` or `assets/images/references/{reference-id}/`, with runtime previews under `assets/images/thumbs/`.
- `references.html` is a backlog/research page. Only user-approved ISM candidates should move from `assets/data/references.json` into `assets/data/isms.json`.
...
│   ├── images/references/{reference-id}/overview.png
│   ├── images/thumbs/{ism-id}/*.webp
│   ├── images/thumbs/effects/{effect-id}/guide.webp
│   ├── images/thumbs/references/{reference-id}/overview.webp
│   └── js/
│       ├── effects-demos.js
│       ├── effects-docs.js
│       ├── references.js
...
- Edit `src/references.ts`, then run `npm run build` for `assets/js/references.js`.
...
The current expansion batch generated 32 PNG originals and `npm run images:thumbs` generated matching WebP previews.
...
- Edit reference and ISM-candidate backlog data in `assets/data/references.json`.
...
- Add reference overview images under `assets/images/references/{reference-id}/overview.png`.
