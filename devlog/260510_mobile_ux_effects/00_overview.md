...
- [x] `effects.html` 별도 정적 페이지를 추가한다.
- [x] `assets/data/effects.json`으로 효과 데이터를 분리한다.
- [x] 기존 ISM 보드와 같은 시각 언어, 카드, 모달, 토스트, 라이트박스 감각을 유지한다.
- [x] 각 효과 카드는 작은 live demo를 가진다.
- [x] 카드 클릭 시 기존 ISM 모달 계열의 상세 팝업을 연다.
- [x] 사용자가 효과 이름을 몰라도 찾도록 `alsoCalled` 별칭 필드를 둔다.
- [x] GitHub Pages에서 빌드 산출물만으로 동작한다.
- [x] 모바일 접근성, reduced motion, 성능 기준을 효과별로 설명한다.
- [x] ima2식 guide image와 WebP preview를 모든 46개 후보군에 붙인다.
- [x] 46개 후보군마다 고유 `demo.type`과 전용 CSS demo animation을 둔다.
...
| Images | original PNG + generated WebP thumbnail | effects guide도 WebP preview + original PNG lightbox 원칙을 따른다 |
...
│   ├── css/effects-demos-candidates.css # NEW: per-candidate extended demos
│   ├── data/
│   │   └── effects.json                 # NEW: effect data
│   ├── images/effects/
│   │   ├── bottom-sheet/guide.png       # original PNG
│   │   ├── swipe-action/guide.png       # original PNG
│   │   └── ...
│   ├── images/thumbs/effects/
│   │   ├── bottom-sheet/guide.webp      # generated WebP preview
│   │   └── ...
│   ├── js/effects-demos.js              # GENERATED from src/effects-demos.ts
│   └── js/effects.js                    # GENERATED from src/effects.ts
├── src/
│   ├── effects-demos.ts                 # NEW: 46 demo type registry
│   └── effects.ts                       # NEW: effects page runtime
├── scripts/
│   └── generate-thumbnails.mjs          # existing PNG -> WebP generator reused
...
| Phase | Doc | Output |
| --- | --- | --- |
| 1 | [01_data_content.md](01_data_content.md) | `effects.json` schema, 46 data rows, naming rules |
...
| 4 | [04_image_guide_pipeline.md](04_image_guide_pipeline.md) | ima2 guide image spec and WebP thumbnail implementation |
