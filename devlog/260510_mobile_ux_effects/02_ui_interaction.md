...
- [ ] `effects.html` 새 페이지를 추가한다.
- [ ] `src/effects.ts`에서 효과 데이터 로드, 필터, 검색, modal을 처리한다.
- [ ] `assets/js/effects.js`가 build 산출물로 생성된다.
- [ ] 기존 `index.html` 헤더에 `Effects` 링크를 추가한다.
- [ ] 기존 ISM modal CSS 계열을 재사용하되 effect 상세에 맞게 확장한다.
...
Each effect chooses one `demo.type`. The renderer maps it to small HTML/CSS.

| `demo.type` | Demo |
| --- | --- |
| `bottom-sheet` | phone frame with rising sheet |
| `full-modal` | screen overlay fills phone frame |
| `drawer` | side panel slides over phone |
| `sticky-cta` | bottom bar sticks inside phone |
| `scroll-reveal` | cards fade upward |
| `staggered-cards` | three cards appear sequentially |
| `press-scale` | button presses down on hover/touch |
| `swipe-action` | list item exposes delete/action area |
| `skeleton` | shimmering placeholder |
| `toast` | small message appears then fades |
| `segmented-control` | active pill slides |
| `lightbox` | thumbnail expands indicator |
...
- Add effect-specific classes only
