...
(function (RecipeChooser) {
    ...
    function preview(snapshot, composition) {
        ...
        if (!style || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(style.ref.id))
            return { src: null, label };
        const entry = DesignCatalog.resolve(snapshot, style.ref);
        const images = entry.images;
        const first = Array.isArray(images) ? images[0] : null;
        const file = first && typeof first === 'object' && !Array.isArray(first)
            ? first.file : null;
        if (typeof file !== 'string' || !/^[a-zA-Z0-9_-]+\.png$/.test(file))
            return { src: null, label };
        return { src: 'assets/images/thumbs/' + style.ref.id + '/' + file.replace(/\.png$/, '.webp'), label };
    }
    function choices(data, recipe, lang) {
    ...
    function mount({ root, getLang, openIsm }) {
        ...
        const body = root.querySelector('.recipe-body');
        if (!body)
            throw new Error('RecipeChooser requires .recipe-body');
        const content = body;
        let lang = getLang();
        let state = 'idle';
        let notice = 'idle';
        ...
        function focusTarget(key) {
            return Array.from(workbench.querySelectorAll('[data-focus]'))
        ...
        function paint() {
            ...
            const disclosure = content.querySelector('[data-detail="guidance"]');
            if (disclosure)
                guidanceOpen = disclosure.open;
            let ready = null;
            if (state === 'ready' && data) {
        ...
        function onToggle(event) {
            if (disposed || event.target !== workbench)
                return;
            RecipeView.localizeSummary(workbench, lang);
            if (workbench.open) {
                void load();
        ...
        function onChange(event) {
            if (disposed || state !== 'ready' || !data)
                return;
            const target = event.target;
            if (target instanceof HTMLInputElement && target.name === 'recipe-choice') {
                if (!target.checked || !data.recipes.some(recipe => recipe.id === target.value))
                    return;
                ...
                return;
            invalidateCopy();
            notice = manual ? 'manual' : 'updated';
            paint();
        }
        async function copy() {
            if (disposed || copying || !composition || !workbench.open)
                return;
            ...
            try {
                ...
                if (disposed || token !== copyGeneration || !workbench.open)
                    return;
                copying = false;
                notice = 'copied';
                RecipeView.feedback(content, messages[notice][lang], copying);
                ...
                notice = 'manual';
                paint();
                const textarea = content.querySelector('.recipe-manual-copy textarea');
                if (mayFocus && textarea) {
        ...
        function onClick(event) {
            ...
            const link = event.target.closest('a[data-domain="isms"][data-id]');
            ...
            if (!id || !composition?.slots.some(slot => slot.ref.domain === 'isms' && slot.ref.id === id))
                return;
            if (copying) {
                invalidateCopy();
                RecipeView.feedback(content, messages[notice][lang], copying);
        ...
        function onImageError(event) {
            ...
            if (!disposed && image instanceof HTMLImageElement && content.contains(image)
                && image.closest('.recipe-preview'))
                RecipeView.imageFailed(image, lang);
        }
        function setLang(next) {
            if (disposed || next === lang || (next !== 'ko' && next !== 'en'))
                return;
            lang = next;
            invalidateCopy();
            paint();
