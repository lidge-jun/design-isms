...
(() => {
    ...
    async function init() {
        const grid = CatalogShell.getRequiredElement('#layout-grid');
        try {
            ...
            CatalogShell.getRequiredElement('#layout-result-count').textContent = 'Error';
    ...
    function mount(grid) {
        const searchInput = CatalogShell.getRequiredElement('#layout-search');
        const filterRow = CatalogShell.getRequiredElement('#layout-family-filter');
        shell = CatalogShell.mount({
            elements: {
                grid,
                resultCount: CatalogShell.getRequiredElement('#layout-result-count'),
                modalOverlay: CatalogShell.getRequiredElement('#layout-modal-overlay'),
                modalDialog: CatalogShell.getRequiredElement('#layout-modal-dialog'),
                modalClose: CatalogShell.getRequiredElement('#layout-modal-close'),
                modalContent: CatalogShell.getRequiredElement('#layout-modal-content'),
                lightbox: document.querySelector('#layout-lightbox'),
                lightboxClose: document.querySelector('#layout-lightbox-close'),
                lightboxImage: document.querySelector('#layout-lightbox-image')
            },
        ...
        document.querySelector('#layout-modal-content')?.addEventListener('click', (event) => {
            const image = event.target instanceof Element ? event.target.closest('.layout-guide-image') : null;
            if (image)
    ...
    function wireCopyButtons(dialog) {
        dialog.querySelectorAll('.layout-copy-snippet').forEach((button) => {
    ...
    function matches(pattern) {
        if (familyFilter !== 'all' && pattern.family !== familyFilter)
            return false;
        if (!query)
            return true;
    ...
    function renderViewportColumn(pattern, viewport, label, reference) {
        const wire = LayoutWireframes.isWireframeType(pattern.wireframe.type)
    ...
    function renderModal(pattern) {
        const slots = pattern.composition.map((slot) => `<li><code>${esc(slot.role)}</code> ${esc(slot.labelKr)}${slot.required ? '' : ' <span class="layout-slot-optional">(선택)</span>'}</li>`).join('');
        const rules = pattern.responsive.map((rule) => `<li><strong>${esc(rule.from)} → ${esc(rule.to)}</strong> ${esc(rule.rule)}</li>`).join('');
        const chips = (items) => items.map((item) => `<span class="layout-chip">${esc(item)}</span>`).join('');
