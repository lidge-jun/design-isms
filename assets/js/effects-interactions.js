 ...
 */
var EffectsInteractions;
(function (EffectsInteractions) {
    ...
    function mount(root) {
        ...
        function applyPointer(event) {
            ...
            if (!(target instanceof Element))
                return;
            const magnetic = target.closest('.demo-magnetic');
            if (magnetic) {
            ...
            const tilt = target.closest('.demo-tilt-card');
            if (tilt) {
        ...
        function onPointerLeave(event) {
            ...
            if (!(target instanceof Element))
                return;
            const el = target.closest('.demo-magnetic span, .demo-magnetic, .demo-tilt-card');
            if (el instanceof HTMLElement) {
                const inner = el.matches('.demo-magnetic') ? el.querySelector('span') : el;
                if (inner)
        ...
        function onClick(event) {
            ...
            if (!(target instanceof Element))
                return;
            const action = target.closest('[data-demo-action]');
            if (!action)
                return;
            ...
            if (kind === 'favorite') {
                ...
                const live = action.parentElement?.querySelector('[aria-live]');
                if (live)
    ...
    EffectsInteractions.mount = mount;
})(EffectsInteractions || (EffectsInteractions = {}));
