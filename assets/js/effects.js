...
(() => {
    ...
    function readDemo(value, effectId) {
        ...
        const demoTypes = ['bottom-sheet', 'full-modal', 'drawer', 'sticky-cta', 'scroll-reveal', 'staggered-cards', 'press-scale', 'swipe-action', 'skeleton', 'toast', 'segmented-control', 'lightbox'];
        if (!demoTypes.includes(type)) {
            throw new Error(`${effectId}.demo.type is invalid`);
        }
        return { type: type, label: readString(record, 'label') };
    }
