...
(function (MotionInteractions) {
    ...
    class Lifetime {
        ...
        animate(node, frames, duration, easing = EXIT_EASE) {
            this.cancel(node);
            if (this.disposed || this.media.matches)
                return;
    ...
    function progress(root, life, uid) {
      ...
      <div class="mi-range-ends" aria-hidden="true"><span>0%</span><span>100%</span></div>`;
        const range = root.querySelector('.mi-range');
        const output = root.querySelector('output');
        const bar = root.querySelector('[role="progressbar"]');
        const fill = root.querySelector('.mi-progress-fill');
    ...
    function scrollReveal(root, life, uid) {
        const entries = [
      ...
      <div class="mi-scroll" role="region" tabindex="0" aria-labelledby="${uid}-title" aria-describedby="${uid}-hint">
        ...
        const viewport = root.querySelector('.mi-scroll');
        const items = [...root.querySelectorAll('.mi-reveal-item')];
        if (life.media.matches || typeof IntersectionObserver === 'undefined')
            return;
        items.forEach(item => { item.dataset.revealed = 'false'; });
        const observer = new IntersectionObserver(entries => {
            if (life.disposed || life.media.matches)
                return;
            entries.forEach(entry => {
                const item = entry.target;
                if (!entry.isIntersecting || item.dataset.revealed === 'true')
                    return;
    ...
    function disclosure(root, life, uid) {
      ...
      <div class="mi-disclosure-shell"><div class="mi-disclosure-body" id="${uid}-details" hidden>
        ...
        const button = root.querySelector('.mi-disclosure-toggle');
        const action = root.querySelector('.mi-disclosure-action');
        const shell = root.querySelector('.mi-disclosure-shell');
        const body = root.querySelector('.mi-disclosure-body');
    ...
    function reorder(root, life) {
      ...
      <ol class="mi-reorder-list" role="list" aria-label="읽는 순서">
        ...
        const button = root.querySelector('.mi-reorder-toggle');
        const list = root.querySelector('.mi-reorder-list');
        const status = root.querySelector('.mi-status');
        life.listen(button, 'click', () => {
    ...
    MotionInteractions.mount = mount;
})(MotionInteractions || (MotionInteractions = {}));
