...
(function (RecipeView) {
    ...
    function refKey(ref) { return ref.domain + '/' + ref.id; }
    RecipeView.refKey = refKey;
    function name(item, lang) {
    ...
    function catalogLink(ref, text, focus) {
        const pages = {
    ...
    function localizeSummary(root, lang) {
        const ko = lang === 'ko';
        ...
        if (summary)
            summary.dataset.focus = 'workbench';
    }
    RecipeView.localizeSummary = localizeSummary;
    function options(ready, lang) {
        ...
        for (const recipe of ready.recipes) {
            ...
            input.type = 'radio';
            input.name = 'recipe-choice';
            input.value = recipe.id;
            input.checked = recipe.id === ready.recipe.id;
            input.dataset.focus = 'recipe:' + recipe.id;
            label.append(input, element('span', 'recipe-option-title', recipe.title[lang]), element('span', 'recipe-option-summary', recipe.summary[lang]));
    ...
    function unavailable(label, lang) {
        return label + (lang === 'ko' ? ' — 이미지를 불러올 수 없습니다.' : ' — Image unavailable.');
    }
    function imageFailed(image, lang) {
        const figure = image.closest('.recipe-preview');
        const caption = figure?.querySelector('figcaption');
        if (caption)
            caption.textContent = unavailable(image.alt, lang);
        image.remove();
    }
    RecipeView.imageFailed = imageFailed;
    function slots(ready, lang) {
        ...
        for (const slot of ready.recipe.slots) {
            ...
            select.dataset.focus = 'slot:' + slot.id;
    ...
    function guidance(ready, lang, open) {
        const details = element('details', 'recipe-more');
        details.dataset.detail = 'guidance';
        details.open = open;
        ...
        for (const section of [
            ...
            for (const text of section.values)
        ...
        for (const [index, source] of ready.composition.sources.entries()) {
            ...
            link.dataset.focus = 'citation:' + index;
    ...
    function detail(ready, model) {
        const lang = model.lang;
        const panel = element('div', 'recipe-detail');
        const heading = element('div', 'recipe-detail-heading');
        heading.append(element('h2', 'recipe-title', ready.recipe.title[lang]), element('p', 'recipe-description', ready.recipe.summary[lang]));
        ...
        if (model.manualCopy !== null) {
            ...
            textarea.id = 'recipe-manual-' + ready.recipe.id;
            label.htmlFor = textarea.id;
            textarea.readOnly = true;
            textarea.rows = 8;
            textarea.value = model.manualCopy;
            textarea.dataset.focus = 'manual-copy';
    ...
    function render(body, model) {
        ...
        if (model.state === 'ready' && model.ready) {
            ...
            error.append(element('p', '', model.lang === 'ko'
                ...
                : 'The composition materials could not load. Please try again.'), button('retry', model.lang === 'ko' ? '다시 시도' : 'Retry'));
            body.insertBefore(error, status);
        }
        feedback(body, model.status, model.copying);
    }
    RecipeView.render = render;
