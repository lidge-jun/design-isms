...
(() => {
    ...
    async function init() {
        const grid = CatalogShell.getRequiredElement('#typography-grid');
        try {
            ...
            if (!Array.isArray(raw))
                throw new Error('typography.json must be an array');
            allPairings = raw;
            mount(grid);
        }
        catch (error) {
            console.error('[typography] failed to initialize', error);
            CatalogShell.getRequiredElement('#typography-result-count').textContent = 'Error';
    ...
    function fontStack(role, koreanFirst) {
    ...
    function specimenTexts(pairing) {
    ...
    function loadPairingFonts(pairing, dialog) {
        ...
        void TypographyFonts.ensureLoaded(pairing.id, base, specimenTexts(pairing)).then((ok) => {
            ...
            const badge = target.querySelector(`[data-font-status="${pairing.id}"]`);
            if (badge)
                badge.textContent = ok ? '웹폰트 로드됨' : '시스템 대체 글꼴 표시 중';
        });
    }
    function mount(grid) {
        const searchInput = CatalogShell.getRequiredElement('#typography-search');
        const filterRow = CatalogShell.getRequiredElement('#typography-category-filter');
        shell = CatalogShell.mount({
            elements: {
                grid,
                resultCount: CatalogShell.getRequiredElement('#typography-result-count'),
                modalOverlay: CatalogShell.getRequiredElement('#typography-modal-overlay'),
                modalDialog: CatalogShell.getRequiredElement('#typography-modal-dialog'),
                modalClose: CatalogShell.getRequiredElement('#typography-modal-close'),
                modalContent: CatalogShell.getRequiredElement('#typography-modal-content'),
                lightbox: document.querySelector('#typography-lightbox'),
                lightboxClose: document.querySelector('#typography-lightbox-close'),
                lightboxImage: document.querySelector('#typography-lightbox-image')
            },
        ...
        document.querySelector('#typography-modal-content')?.addEventListener('click', (event) => {
            const image = event.target instanceof Element ? event.target.closest('.typo-guide-image') : null;
            if (image)
    ...
    function matches(pairing) {
        if (categoryFilter !== 'all' && pairing.category !== categoryFilter)
            return false;
        if (!query)
            return true;
    ...
    function renderScaleRows(pairing) {
        const base = 16;
        const steps = [['Display', 4], ['H1', 3], ['H2', 2], ['H3', 1], ['Body', 0], ['Caption', -1]];
