...
(function (DesignExport) {
    ...
    let snippetFetch = null;
    const extraTabs = [];
    /* ── clipboard ─────────────────────────────────────────────────── */
    async function copyText(text, announcement) {
        let ok = false;
        try {
    ...
    DesignExport.copyText = copyText;
    ...
    function deriveSemanticColors(palette, bgFg) {
        ...
        if (cr < 3)
            warnings.push(`Contrast ${cr.toFixed(2)}:1 is below the 3:1 WCAG AA threshold for large text and UI components.`);
        if (cr < 4.5)
            warnings.push(`Body-text contrast ${cr.toFixed(2)}:1 is below the 4.5:1 WCAG AA threshold for normal text.`);
    ...
    /* ── format generators ─────────────────────────────────────────── */
    function generateCSS(ism, sem, pg) {
        const lines = [];
        ...
        if (pg.gutter)
            lines.push(`  --ism-gutter: ${pg.gutter};`);
        if (pg.spaceMin)
            lines.push(`  --ism-space-min: ${pg.spaceMin};`);
        if (pg.spaceMax)
            lines.push(`  --ism-space-max: ${pg.spaceMax};`);
        if (pg.pageMargin)
            lines.push(`  --ism-page-margin: ${pg.pageMargin};`);
        if (pg.lineHeight)
            lines.push(`  --ism-line-height: ${pg.lineHeight};`);
        if (pg.letterSpacing)
            lines.push(`  --ism-letter-spacing: ${pg.letterSpacing};`);
        if (pg.duration)
            lines.push(`  --ism-motion-duration: ${pg.duration};`);
        if (pg.easing)
            lines.push(`  --ism-motion-easing: ${pg.easing};`);
        lines.push('}');
        return lines.join('\n');
    }
    function generateTailwind(ism, sem, pg) {
        const lines = [];
        ...
        if (pg.gutter)
            lines.push(`  --spacing-ism-gutter: ${pg.gutter};`);
        if (pg.spaceMin)
            lines.push(`  --spacing-ism-min: ${pg.spaceMin};`);
        if (pg.spaceMax)
            lines.push(`  --spacing-ism-max: ${pg.spaceMax};`);
        if (pg.easing)
            lines.push(`  --ease-ism: ${pg.easing};`);
        lines.push('}');
        return lines.join('\n');
    }
    function generateTokens(ism, sem, pg) {
        const obj = {
            color: {
                '$type': 'color',
        ...
        sem.raw.forEach(c => {
            obj['color'][c.name] = { '$value': c.hex };
        });
        const dim = {};
        if (pg.gutter)
            dim['gutter'] = { '$type': 'dimension', '$value': pg.gutter };
        if (pg.spaceMin)
            dim['spaceMin'] = { '$type': 'dimension', '$value': pg.spaceMin };
        if (pg.spaceMax)
            dim['spaceMax'] = { '$type': 'dimension', '$value': pg.spaceMax };
        if (Object.keys(dim).length > 0)
            obj['dimension'] = dim;
        const dur = {};
        if (pg.duration)
            dur['standard'] = { '$type': 'duration', '$value': pg.duration };
    ...
    function renderTabs(tabs, prefix, storageKey) {
    ...
    function bindTabs(container) {
        const panel = container.querySelector('.export-panel');
        if (!panel)
            return;
        const storageKey = panel.dataset.exportStorage ?? TAB_KEY;
        const tabs = Array.from(panel.querySelectorAll('[role="tab"]'));
        const panels = Array.from(panel.querySelectorAll('[role="tabpanel"]'));
        ...
        panel.addEventListener('click', (e) => {
            ...
            const panelEl = btn.closest('[role="tabpanel"]');
    ...
    /* ── ISM export mount ──────────────────────────────────────────── */
    function mountIsm(container, ism, guide) {
        ...
        builtinTabs.forEach(tab => {
            const panelEl = container.querySelector('#ism-export-panel-' + tab.id);
            const code = panelEl?.querySelector('code');
            if (code)
        ...
        container.insertAdjacentHTML('beforeend', '<p class="export-contrast-note">Foreground/background contrast: ' + sem.contrastRatio.toFixed(2) + ':1</p>');
        bindTabs(container);
        extraTabs.forEach(reg => {
            const panelEl = container.querySelector('#ism-export-panel-' + reg.id);
            if (panelEl)
                reg.render(panelEl, ism, guide);
        });
    }
    DesignExport.mountIsm = mountIsm;
    ...
    async function loadSnippets() {
        ...
        snippetFetch = fetch(SNIPPET_URL)
            .then(res => { if (!res.ok)
            throw new Error('snippet fetch ' + String(res.status)); return res.json(); })
            .then(raw => {
            const obj = raw;
            snippetCache = (obj['snippets'] ?? obj);
            return snippetCache;
        })
            .catch(() => { snippetFetch = null; return null; });
        return snippetFetch;
    }
    async function mountEffect(container, effectId) {
        ...
        if (entry.js)
            tabs.push({ id: 'js', label: 'JavaScript', content: entry.js });
        const notes = [];
        if (entry.reducedMotion)
            notes.push('Reduced motion: ' + entry.reducedMotion);
        if (entry.a11yNotes)
            entry.a11yNotes.forEach(n => notes.push(n));
        if (entry.sourceRefs)
            entry.sourceRefs.forEach(r => notes.push('Reference: ' + r));
        if (notes.length > 0) {
        ...
        tabs.forEach(tab => {
            const panelEl = container.querySelector('#fx-' + effectId + '-panel-' + tab.id);
            const code = panelEl?.querySelector('code');
            if (code)
                code.textContent = tab.content;
        });
    ...
    DesignExport.registerIsmTab = registerIsmTab;
