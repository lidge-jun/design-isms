...
(function (AppMaterials) {
    ...
    function mount(root, ismId, lang) {
        if (ismId !== 'refractive-glass-ui')
            return;
        const guide = root.querySelector('.modal-dev-guide');
        if (!guide)
            return;
        ...
        guide.before(lab);
        const title = lab.querySelector('.material-lab-scene-title');
        const body = lab.querySelector('.material-lab-scene-body');
        const selection = lab.querySelector('.material-lab-selection');
        const buttons = lab.querySelectorAll('[data-material-scene]');
        const solid = lab.querySelector('input');
