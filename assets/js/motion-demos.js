...
(function (MotionDemos) {
    ...
    const cubicPattern = new RegExp(`^cubic-bezier\\(\\s*(${numberPattern})\\s*,\\s*(${numberPattern})\\s*,\\s*(${numberPattern})\\s*,\\s*(${numberPattern})\\s*\\)$`);
    ...
    function parseEasing(value) {
        ...
        for (const part of parts) {
            ...
            if (!Number.isFinite(stop.value) || !Number.isFinite(stop.position))
                return null;
            if (stop.position < 0 || stop.position > 100)
                return null;
            const previous = stops[stops.length - 1];
            if (previous && stop.position <= previous.position)
                return null;
            stops.push(stop);
        }
        const first = stops[0];
        const last = stops[stops.length - 1];
        if (!first || !last || first.position !== 0 || last.position !== 100)
            return null;
        return { kind: 'linear', stops };
    }
    MotionDemos.parseEasing = parseEasing;
    function curveSvg(easing) {
        const parsed = parseEasing(easing);
        if (!parsed)
            return '<p class="motion-curve-error" role="status">Invalid easing value</p>';
        ...
        const x = (value) => plot.left + value * (plot.right - plot.left);
        const y = (value) => plot.bottom - ((value - minY) / (maxY - minY)) * (plot.bottom - plot.top);
        const point = (px, py) => `${formatNumber(x(px))},${formatNumber(y(py))}`;
    ...
    function isDemoType(value) {
        return MotionDemos.demoTypes.includes(value);
    }
    MotionDemos.isDemoType = isDemoType;
    function cubicCurve(easing, point) {
    ...
    function linearCurve(easing, point) {
