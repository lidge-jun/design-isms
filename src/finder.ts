  ...
  */
 namespace DesignFinder {
   /* ── types ──────────────────────────────────────────────────── */
   interface LangStr { ko: string; en: string; }
   interface Option { id: string; label: LangStr; hint: LangStr; keywords: string[]; guideTerms: string[]; boostIds: string[]; conflictIds: string[]; }
   interface Question { id: string; legend: LangStr; options: Option[]; }
   interface Config { version: string; questions: Question[]; }
   interface FinderIsm { id: string; name: string; nameKr: string; tagline: string; palette: string[]; keywords: string[]; kind?: string; }
   type GuideMap = Record<string, Record<string, unknown>>;
   type Lang = 'ko' | 'en';
   interface MountOpts {
     root: HTMLElement; isms: FinderIsm[]; guides: GuideMap | null;
     getLang: () => Lang; openModal: (id: string, trigger: HTMLElement | null) => void;
   }
   export interface RankedResult { id: string; name: string; nameKr: string; score: number; boostCount: number; kwCount: number; guideCount: number; paletteMatch: boolean; reasons: string[]; caution: string; confidence: string; }
 
   const CONFIG_URL = './assets/data/finder-config.json?v=2026-07-15-atlas49';
   const STORAGE_KEY = 'design-isms-finder-v1';
   let configCache: Config | null = null;
   let configFetch: Promise<Config | null> | null = null;
   ...
   async function loadConfig(): Promise<Config | null> {
     ...
     configFetch = fetch(CONFIG_URL)
       .then(r => { if (!r.ok) throw new Error('config ' + r.status); return r.json() as Promise<Config>; })
   ...
   /* ── scoring ───────────────────────────────────────────────── */
   function flatGuideText(guide: Record<string, unknown>): string {
     const parts: string[] = [];
     for (const val of Object.values(guide)) {
       if (typeof val === 'string') parts.push(val);
       else if (Array.isArray(val)) val.forEach(v => { if (typeof v === 'string') parts.push(v); });
       else if (typeof val === 'object' && val !== null) {
         for (const inner of Object.values(val as Record<string, unknown>)) {
           if (typeof inner === 'string') parts.push(inner);
           else if (Array.isArray(inner)) inner.forEach(v => { if (typeof v === 'string') parts.push(v); });
         }
       }
     }
   ...
   export function score(isms: readonly FinderIsm[], guides: GuideMap | null, config: Config, answers: { project: string; mood: string; brightness: string }): RankedResult[] {
     ...
     for (let idx = 0; idx < isms.length; idx++) {
       const ism = isms[idx]!;
       if (ism.kind === 'anti-pattern') continue;
       let total = 0; let boostCount = 0; let kwCount = 0; let guideCount = 0; let paletteMatch = false;
       const reasons: string[] = [];
 
       // boost: +6 per option whose boostIds contains this ism
       for (const opt of selectedOpts) {
         if (opt.boostIds.includes(ism.id)) { total += 6; boostCount++; reasons.push('direct match: ' + opt.id); }
       }
       // keyword: +3 per unique keyword match
       for (const kw of allKeywords) {
         if (ism.keywords.some(k => k.toLowerCase() === kw.toLowerCase())) { total += 3; kwCount++; if (reasons.length < 4) reasons.push('keyword: ' + kw); }
       }
       ...
       if (guides) {
         ...
         if (guide) {
           ...
           for (const term of allGuideTerms) {
             ...
             if (re.test(text) && gc < 3) { total += 1; gc++; guideCount++; }
           }
         }
       }
       // palette brightness: +3
       const guideColor = guides?.[ism.id] as Record<string, unknown> | undefined;
       const bgFg = typeof (guideColor?.['color'] as Record<string, string> | undefined)?.['bgFg'] === 'string'
         ? ((guideColor?.['color'] as Record<string, string>)['bgFg']!) : '';
       const pc = DesignExport.classifyPalette(ism.palette, bgFg);
       if (pc === brightAns) { total += 3; paletteMatch = true; reasons.push('palette: ' + brightAns); }
       // conflict: -5
       if (allConflicts.has(ism.id)) { total -= 5; }
 
       const confidence = total >= 18 ? 'Strong starting point' : total >= 10 ? 'Worth comparing' : 'Exploratory match';
       const guide = guides?.[ism.id] as Record<string, unknown> | undefined;
       const donts = (guide?.['donts'] as string[] | undefined) ?? [];
   ...
   /* ── mount ─────────────────────────────────────────────────── */
   export function mount(opts: MountOpts): { setLang: (lang: Lang) => void } {
     ...
     try {
       ...
       if (stored) answers = JSON.parse(stored) as Record<string, string>;
     } catch { /* */ }
     ...
     function render(): void {
       ...
       const form = root.querySelector<HTMLFormElement>('#finder-form')!;
       const submitBtn = root.querySelector<HTMLButtonElement>('#finder-submit')!;
       const resetBtn = root.querySelector<HTMLButtonElement>('#finder-reset')!;
       const resultsEl = root.querySelector<HTMLElement>('#finder-results')!;
       ...
       form.addEventListener('submit', (e: Event) => {
         e.preventDefault();
         if (!config) return;
         results = score(isms, guides, config, answers as { project: string; mood: string; brightness: string });
       ...
       resetBtn.addEventListener('click', () => {
         ...
         resultsEl.innerHTML = '';
         const firstRadio = form.querySelector<HTMLInputElement>('.finder-radio');
         firstRadio?.focus();
       });
     ...
     function renderResults(el: HTMLElement, res: RankedResult[], lang: Lang): void {
       ...
       el.innerHTML = html;
       el.querySelectorAll<HTMLButtonElement>('.finder-result-open').forEach(btn => {
     ...
     function setLang(lang: Lang): void {
       if (config && results.length > 0) {
         const resultsEl = root.querySelector<HTMLElement>('#finder-results');
         if (resultsEl) renderResults(resultsEl, results, lang);
       }
       render();
     }
   ...
   function buildPromptPack(ism: DesignExport.IsmInput, guide: DesignExport.GuideInput, _lang: Lang): string {
     const lines: string[] = [];
     lines.push('PROJECT');
     ...
     if (typo['fontPairing']) lines.push(typo['fontPairing'] + '.');
     if (typo['sizeHierarchy']) lines.push(typo['sizeHierarchy'] + '.');
     if (typo['lineHeight']) lines.push('Line height: ' + typo['lineHeight'] + '.');
     lines.push('');
     lines.push('LAYOUT');
     ...
     if (motion['duration']) lines.push('Duration: ' + motion['duration'] + '.');
   ...
   function buildDeslopPrompt(_ism: DesignExport.IsmInput): string {
     const lines: string[] = [];
   ...
   DesignExport.registerIsmTab({
     ...
     render: (container: HTMLElement, ism: DesignExport.IsmInput, guide: DesignExport.GuideInput) => {
       const isAntiPattern = document.querySelector<HTMLElement>('[data-kind="anti-pattern"]') !== null;
       const lang: Lang = (document.documentElement.lang as Lang) || 'ko';
