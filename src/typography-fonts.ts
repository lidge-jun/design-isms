...
namespace TypographyFonts {
  const loaded = new Map<string, Promise<boolean>>();

  function subsetChars(texts: string[]): string {
    const unique = [...new Set(texts.join('').replace(/\s+/g, ' ').split(''))];
    return unique.sort().join('');
  }

  export function ensureLoaded(pairingId: string, baseUrl: string, texts: string[]): Promise<boolean> {
