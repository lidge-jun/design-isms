...
try {
  ...
  await assertCleanDiagnostics(cdp, 'faq flow');
  for (const page of pages) {
...
if (rows.length !== 18 || screenshots.length !== 9 || teardown.browserSessions !== 24 || teardown.serverExitCode !== 0 || teardown.serverForced || !teardown.serverPortClosed || !teardown.cdpPortClosed || !teardown.profileRemoved || teardown.browserStopExitCode !== 0) throw new Error(`browser QA incomplete ${JSON.stringify({ rows: rows.length, screenshots: screenshots.length, teardown })}`);
